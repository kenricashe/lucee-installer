...
generate_site_include() {
	...
	local sites_dir="${UPG_DIR}/sites"
	mkdir -p "$sites_dir"
	
	# Generate include file path
	local include_file="${sites_dir}/${domain}-${port}.conf"
	
	# Create the include file with header
...
EOF
	...
	if [ -n "$error_block" ]; then
		...
		if [ -n "$error_line" ]; then
			echo -e "\t$error_line" >> "$include_file"
		fi
...
EOF
		...
		cat >> "$include_file" << EOF
	# No CF-specific 404 handler found for this site
	# This include file ensures consistent upgrade behavior across all sites
</IfDefine>

<IfDefine LUCEE_UPGRADE_IN_PROGRESS>
	# No CF-specific 404 handler to conditionally enable
</IfDefine>
EOF
	fi
...
# Configure site includes for all sites regardless of CF 404 handlers
configure_site_includes() {
	...
	# Always generate per-site include file (with or without 404 block)
	generate_site_include "$domain" "$port" "$error_block"
	
	# Comment out original 404s if we found CF-specific ones
	if [ -n "$error_block" ]; then
	...
	add_site_include_to_vhost "$conf_file" "$domain" "$port"
	
	# Ensure detection include is present
	ensure_include_in_vhost "$conf_file" "$domain" "$port"
}
...
add_site_include_to_vhost() {
	local vhost_file="$1"
	local domain_match="$2"
	local port_filter="$3"
	local sites_dir="${UPG_DIR}/sites"
	local include_file="${sites_dir}/${domain_match}-${port_filter}.conf"
...
# Create Apache Directory directive to ensure upgrade-in-progress.html is accessible
create_directory_directive() {
	local docroot=$1
	local dir_conf="${CONF_DIR}/lucee-upgrade-directory.conf"
	local tmp="$(mktemp)"
	
	# Create Directory directive for the specific file
	{
		echo "# Generated by Lucee upgrade script - DO NOT EDIT"
		echo "# This file ensures the upgrade-in-progress.html file is accessible during upgrades"
		echo "<Directory \"${docroot}\">"
		echo "\t<Files \"upgrade-in-progress.html\">"
	...
	# Ensure the directory directive is included in Apache config
	if [ "$IS_DEBIAN" = true ]; then
		# For Debian/Ubuntu, create symlink in conf-enabled
		if [ ! -L "/etc/apache2/conf-enabled/lucee-upgrade-directory.conf" ]; then
			ln -sf "$dir_conf" "/etc/apache2/conf-enabled/lucee-upgrade-directory.conf"
			echo "Created Apache Directory directive for upgrade-in-progress.html"
			RELOAD_APACHE=true
		fi
	elif [ "$IS_CPANEL" = true ]; then
		# For cPanel, include in main config if not already included
		if ! grep -q "Include.*lucee-upgrade-directory.conf" "/usr/local/apache/conf/includes/pre_main_global.conf" 2>/dev/null; then
			echo "Include \"$dir_conf\"" >> "/usr/local/apache/conf/includes/pre_main_global.conf"
			echo "Added Directory directive include to cPanel Apache config"
			RELOAD_APACHE=true
		fi
	else
		# For RHEL/CentOS, include in main config if not already included
		if ! grep -q "Include.*lucee-upgrade-directory.conf" "$CONF_DIR/conf.d/lucee.conf" 2>/dev/null; then
			# Create lucee.conf if it doesn't exist
			if [ ! -f "$CONF_DIR/conf.d/lucee.conf" ]; then
				echo "# Lucee configuration" > "$CONF_DIR/conf.d/lucee.conf"
			fi
			echo "Include \"$dir_conf\"" >> "$CONF_DIR/conf.d/lucee.conf"
			echo "Added Directory directive include to Apache config"
			RELOAD_APACHE=true
		fi
	fi
}
...
copy_upgrade_html() {
	...
	# Create a Directory directive to ensure Apache can access the file
	create_directory_directive "$docroot"
}
...
configure_site_debian() {
	...
	if [ -f "$ssl_conf_file" ]; then
		...
		if has_site_include "$domain" "443"; then
			...
			add_site_include_to_vhost "$ssl_conf_file" "$domain" "443"
	...
	if [ -f "$http_conf_file" ]; then
		...
		if has_site_include "$domain" "80"; then
			...
			add_site_include_to_vhost "$http_conf_file" "$domain" "80"
...
configure_site_cpanel() {
	...
	local sites_dir="${UPG_DIR}/sites"
	local ssl_include="${sites_dir}/${domain}-443.conf"
	local http_include="${sites_dir}/${domain}-80.conf"
