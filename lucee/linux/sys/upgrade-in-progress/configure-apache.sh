...
generate_allowed_ip_proxy_include() {
	...
	local ppm_opts
	ppm_opts=""
	if [ "$backend_type" = "ajp" ]; then
		# Try to extract secret=... from lucee-proxy.conf (ProxyPass/ProxyPassMatch/BalancerMember)
		local ajp_secret
		ajp_secret=$(echo "$content" | sed -n 's/.*secret=\([^[:space:]]\+\).*/\1/p' | head -1)
		if [ -z "$ajp_secret" ]; then
			echo "Error: AJP backend detected but no secret found in $src."
			echo "Please configure a secure AJP secret in your active lucee-proxy.conf (ProxyPass/ProxyPassMatch or BalancerMember)."
			rm -f "$tmp"
			return 1
		fi
		ppm_opts=" secret=${ajp_secret}"
	fi

	# Generate internal mapping plus rewrite-gated access for allowlisted IPs
	{
